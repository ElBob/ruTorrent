...
файловой системе хоста.
  ...
  Три флажка в настройках AutoMove управляют дополнительными подкаталогами,
добавляемыми к пути назначения перед сохранением файлов:

  "Добавить метку торрента к пути"   — добавляет метку торрента.
  "Добавить трекер торрента к пути"  — добавляет доменное имя трекера
                                       (например, "https://tracker.example.com/announce"
                                       становится "example.com").
  "Добавить имя торрента к пути"     — добавляет имя торрента.

  Они применяются в указанном порядке (метка → трекер → имя). Например,
при включении всех трёх и базовом каталоге назначения /media/p2p/, торрент
с меткой "Movies", трекером https://tracker.example.com/announce,
именем "MyMovie", загружаемый в /usr/p2p/downloads/Video/DVD/, будет
перемещён в:
...
-------------------------------------------------------------------------------
    ...
    2026-04-25:
    - AutoMove: добавлена опция "Добавить трекер к пути" — добавляет домен
      трекера как подкаталог пути назначения
